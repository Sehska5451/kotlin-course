...
// когда нужно указать только фиксированное смещение от координированного всемирного времени,
// без перехода на летнее время и других изменений. Для указания смещения используется класс ZoneOffset.
// Он подходит для случаев, когда известно только смещение от UTC и не требуется учет дополнительных правил часового пояса.
...
//
////Сравнение дат
//    date1.isAfter(date2) // Проверяет, больше ли date1, чем date2
//    date1.isBefore(date2) // Проверяет, меньше ли date1, чем date2
//    date1.isEqual(date2) // Проверяет, равны ли date1 и date2

//ChronoUnit
//enum, представляющий единицу измерения времени, такую как дни, месяцы, годы.
// Используется в операциях с датами для указания, в каких единицах измерения производится добавление или вычитание.
...
// но они используются в различных контекстах и имеют разные основные единицы измерения.
